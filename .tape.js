...
module.exports = {
	...
	'basic:export': {
		...
		after() {
			if (global.__exportPropertiesObject.customProperties['--color'] !== 'rgb(255, 0, 0)') {
	...
	'basic:export-json': {
		...
		after() {
			const comparePropertiesString = fs.readFileSync('test/export-properties.json', 'utf8');

			if (global.__exportPropertiesString !== comparePropertiesString) {
				throw new Error(
					`The original file did not match the freshly exported copy\nExpected: ${JSON.stringify(global.__exportPropertiesString)}\nReceived: ${JSON.stringify(comparePropertiesString)}`
				);
			}
		}
	},
	'basic:export-js': {
		...
		after() {
			const comparePropertiesString = fs.readFileSync('test/export-properties.js', 'utf8');
	...
	'basic:export-mjs': {
		...
		after() {
			const comparePropertiesString = fs.readFileSync('test/export-properties.mjs', 'utf8');
	...
	'basic:export-css': {
		...
		after() {
			const comparePropertiesString = fs.readFileSync('test/export-properties.css', 'utf8');
	...
	'basic:export-css-to-type': {
		...
		after() {
			...
			if (global.__exportPropertiesString !== comparePropertiesString) {
				throw new Error(
					`The original file did not match the freshly exported copy\nExpected: ${global.__exportPropertiesString}\nReceived: ${comparePropertiesString}`
				);
			}
		}
	},
